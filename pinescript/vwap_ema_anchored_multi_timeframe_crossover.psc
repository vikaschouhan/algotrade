//@version=4
strategy("VWAP-EMA MTF crossover", overlay=true)

///////////////////////////////////
/// Inputs
time_frame   = input(defval="D", type=input.resolution,     title="Time frame")
vwap_ema     = input(1,          type=input.integer,        title='VWAP EMA Lookback period')
vwap_src     = input(hl2,        type=input.source,         title='VWAP Source')
...
use_tstop    = input(true,       type=input.bool,           title='Use trailing stop')
...
vwap_sum_fn(src) =>
    vwap_sum = 0.0
    vwap_sum := (new_session == 1) ? (src*volume) : (vwap_sum[1]+src*volume)
    vwap_sum
//
vol_sum_fn() =>
    vol_sum = 0.0
...
////////////////////////////////////////
/// SIgnals
src      = ema(vwap_src, vwap_ema)
vwap_sig = vwap_sum_fn(src)/vol_sum_fn()
...
////////////////////////////////////////
/// Plots
plot(vwap_sig, title="VWAP-EMA", color=color.blue)
