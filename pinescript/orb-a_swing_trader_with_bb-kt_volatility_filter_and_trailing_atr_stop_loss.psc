//@version=4
strategy(title="ORB-A SwingTrader with BB-KT volatility filter", overlay=true)
...
// vol filter params
vo_source           = input(defval=close,   title='BB Volatility Source', type=input.source)
vo_length           = input(defval=20,      title='BB Volatility Loopback Period Length', type=input.integer, minval=1)
vo_mult             = input(defval=2.0,     title='BB Volatility ATR Multiplier', type=input.float, minval=0.01, maxval=50, step=0.01)
vo_tf               = input(defval='1D',    title='BB Volatility Timeframe', type=input.resolution)
vo_vol_el           = input(defval=9,       title='BB Volatiltiy EMA Loopback period Length', type=input.integer)
...
// Bollinger Bands
bb_basis    = sma(vo_source, vo_length)
bb_dev      = vo_mult * stdev(vo_source, vo_length)
bb_upper    = bb_basis + bb_dev
bb_lower    = bb_basis - bb_dev
// Keltener Channel
kt_range    = tr
kt_rangema  = sma(kt_range, vo_length)
kt_upper    = bb_basis + kt_rangema * vo_mult
kt_lower    = bb_basis - kt_rangema * vo_mult
// Vol Calc
vol_ind       = (bb_upper - kt_upper)
vol_ind_e     = ema(vol_ind, vo_vol_el)
ind_vol_t     = security(syminfo.tickerid, vo_tf, vol_ind)
ind_vol_ema_t = security(syminfo.tickerid, vo_tf, vol_ind_e)
