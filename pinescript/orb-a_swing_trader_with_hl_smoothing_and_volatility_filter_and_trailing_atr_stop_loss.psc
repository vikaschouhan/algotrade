//@version=4
strategy(title="ORB-A HL Smooth SwingTrader with volatility filter", overlay=true)
...
orb_ema_len         = input(defval=1,       title='ORB Smooth EMA Length', type=input.integer)
...
orb_high_rangeL = security(syminfo.tickerid, orb_time_frame, ema(orb_high_range, orb_ema_len))
orb_low_rangeL  = security(syminfo.tickerid, orb_time_frame, ema(orb_low_range, orb_ema_len))
