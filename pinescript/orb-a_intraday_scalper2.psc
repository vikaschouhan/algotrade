//@version=4
strategy(title="ORB-A intraday scalping 2", shorttitle="ORB-A intraday scalping 2", overlay=true)
...
tol_lag   = input(0,     title='Tolerance lag in bars', type=input.integer)
sig_lag   = input(0,     title='Position signal lag in bars', type=input.integer)
...
buy    = use_iday ? (crossover(close[sig_lag], tolbu[sig_lag+tol_lag]) and (hour < end_hr)) : crossover(close[sig_lag], tolbu[sig_lag+tol_lag])
sell   = use_iday ? (crossunder(close[sig_lag], stop_l[sig_lag+1]) or chk_close_time(end_hr, end_min)) : crossunder(close, stop_l[sig_lag+1])
short  = use_iday ? (crossunder(close[sig_lag], tolbl[sig_lag+tol_lag]) and (hour < end_hr)) : crossunder(close, tolbl[sig_lag+tol_lag])
cover  = use_iday ? (crossover(close[sig_lag], stop_s[sig_lag+1]) or chk_close_time(end_hr, end_min)) : crossover(close[sig_lag], stop_s[sig_lag+1])
