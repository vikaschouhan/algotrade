//@version=4
strategy(title="ORB-A intraday scalping with donchian filter", overlay=true)

time_frame_d = input(defval='D', title='Timeframe for donchian channel', type=input.resolution)
donch_len    = input(defval=5, title='Donchian Channel Length', type=input.integer)
...
donch_hi_s = highest(donch_len)
donch_lo_s = lowest(donch_len)
donch_hi   = security(syminfo.tickerid, time_frame_d, donch_hi_s)
donch_lo   = security(syminfo.tickerid, time_frame_d, donch_lo_s)
...
buy    := buy and (close > donch_hi[1])
short  := short and (close < donch_lo[1])
...
plot(donch_hi,     color=color.blue, linewidth=1, title='Donch_HI')
plot(donch_lo,     color=color.olive, linewidth=1, title='Donch_LO')
