//@version=5
strategy("Candles breakout from Range - Close cross below Donchian Low", overlay=true, default_qty_type=strategy.percent_of_equity, default_qty_value=100)

time_frame       = input.timeframe(defval='', title='Time Frame')
donch_time_frame = input.timeframe(defval='', title='Donch Time Frame')
range_thr        = input.float(defval=10, title='Engulfing candle range in %')
donch_length     = input.int(defval=4, title='Donchian Length')
...
donch_lo       = ta.lowest(low, donch_length)
donch_lo_sig   = request.security(syminfo.tickerid, donch_time_frame, donch_lo)

buy_sig   = final_logic //and (roc_pct_sig > 50)
sell_sig  = ta.crossunder(close, donch_lo_sig[1])
