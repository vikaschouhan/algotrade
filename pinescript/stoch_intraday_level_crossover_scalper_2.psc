//@version=4
strategy("Stochastic Intraday Crossover Scalper 2", overlay=false)
...
squareoff_cond     = input('K-D cross', title='Square off condition',type=input.string, options=['K-D cross', 'Stop loss', 'All'])
...
buy_sig_m    = crossover(k_sig, d_sig) and (k_sig < os_level)
short_sig_m  = crossunder(k_sig, d_sig) and (k_sig > ob_level)
sell_sig_m   = false
cover_sig_m  = false
if squareoff_cond == 'K-D cross'
    sell_sig_m  := short_sig_m
    cover_sig_m := buy_sig_m
//
if squareoff_cond == 'Stop loss'
    sell_sig_m  := crossunder(close, stop_l[1])
    cover_sig_m := crossover(close, stop_s[1])
//
if squareoff_cond == 'All'
    sell_sig_m  := short_sig_m or crossunder(close, stop_l[1])
    cover_sig_m := buy_sig_m or crossover(close, stop_s[1])
//
