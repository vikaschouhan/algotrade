//@version=4
strategy(title="ORB-A SwingTrader with volatility filter and atr stop loss v2", overlay=true)
...
strend_() =>
    tup_  = 0.0
    tdn_  = 0.0
    up_   = hl2 - (stop_atr_mult * atr(stop_atr_period))
    dn_   = hl2 + (stop_atr_mult * atr(stop_atr_period))
    tup_  := (stop_src[1] > tup_[1]) ? max(up_, tup_[1]) : up_
    tdn_  := (stop_src[1] < tdn_[1]) ? min(dn_, tdn_[1]) : dn_
    [tup_, tdn_]
//
[stop_up, stop_dn] = strend_()
...
sell_stdalone   = use_stop_loss ? crossunder(close, stop_up[1])[pos_lag] : crossunder(close, orb_tolbl)[pos_lag]
short_stdalone  = crossunder(close, orb_tolbl)[pos_lag]
cover_stdalone  = use_stop_loss ? crossover(close, stop_dn[1])[pos_lag] : crossover(close, orb_tolbu)[pos_lag]
...
plot(stop_up,         color=color.black, linewidth=1)
plot(stop_dn,         color=color.black, linewidth=1)
