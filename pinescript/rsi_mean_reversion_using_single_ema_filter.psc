//@version=5
strategy("RSI mean reversion with single ema filter", overlay=false)
...
sell  = ta.crossover(rsi_sig, 100-rsi_threshold) or ta.crossunder(close, ema_sig)
short = ta.crossover(rsi_sig, 100-rsi_threshold) and (close < ema_sig)
cover = ta.crossunder(rsi_sig, rsi_threshold) or ta.crossover(close, ema_sig)
