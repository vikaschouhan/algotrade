//@version=4
strategy("r1-s1 swing breakout with ema smoothing", overlay=true)
...
ema_len        = input(defval=9,          title='Smmoth EMA for pivots', type=input.integer)
...
pivot_cpr(time_frame) =>
    prev_close     = ema(security(syminfo.tickerid, time_frame, close[1], lookahead=true), ema_len)
    prev_open      = ema(security(syminfo.tickerid, time_frame, open[1], lookahead=true), ema_len)
    prev_high      = ema(security(syminfo.tickerid, time_frame, high[1], lookahead=true), ema_len)
    prev_low       = ema(security(syminfo.tickerid, time_frame, low[1], lookahead=true), ema_len)
